...
public class GameInformation : MonoBehaviour {
	...
	void Start () {
		...
		if (!isControlledByPlayer) {
			...
			GameSubInformation info = JsonUtility.FromJson<GameSubInformation>(playerInputRecord); 
			this.seed = info.seed;
	...
	public class PlayerInput{
		...
		public PlayerInput(ADLBaseAgent player, float horizontalAxis, bool isFireButtonDown, bool isJumpButtonDown, bool isJumpButtonUp) {
			this.x = player.GetComponent<Rigidbody2D>().position.x;
			this.y = player.GetComponent<Rigidbody2D>().position.y;
	...
	public class GameSubInformation {
		public int seed;
